 ...
 * before GC is run for the given owner object (not reachable in user code). but only if the cleaner
 * has not been already run explicitly by user before. If the cleaner is run after GC it will not
 * run clean operation again. This way we guarantee that the clean operation will always run at some
 * point but only once.
 *
...
public enum JavaGcCleanerWrapper {
    ...
    private static CleanerProvider createLegacyCleanerProvider() {
        ...
        return new CleanerProvider(
                ...
                new CleanerFactoryProvider(
                        ...
                        reflectionUtils,
                        cleanerClassName,
                        Optional::empty, // there is no Cleaner object, static method of its class
    ...
    private static CleanerProvider createJava9CleanerProvider() {
        ...
        //     java.lang.ref.Cleaner.Cleanable cleanable = jvmCleaner.register(owner,
        // cleanOperation);
                        ...
                        () -> {
                            ...
                            try {
                                ...
                                throw new FlinkRuntimeException(
                                        "Failed to create a Java 9 Cleaner", e);
                            }
                        },
                        "register",
    ...
    private static CleanerManager findGcCleanerManager() {
        ...
        if (foundCleanerManager == null) {
            String errorMessage =
                    String.format(
                            "Failed to find GC Cleaner among available providers: %s",
                            CLEANER_PROVIDERS);
    ...
    private static class CleanerManager {
        ...
        @Nullable private final PendingCleanersRunner pendingCleanersRunner;
    ...
    private static class CleanerFactoryProvider {
        ...
        private CleanerFactory createCleanerFactory() {
            ...
            return new CleanerFactory(
                    ...
                    cleanerSupplier
                            .get()
                            .orElse(null), // static method of Cleaner class will be called to
                    // create Cleanable
        ...
        private Runnable create(Object owner, Runnable cleanupOperation) {
            ...
            return () -> {
                try {
                    ...
                    String message =
                            String.format(
                                    "FATAL UNEXPECTED - Failed to invoke a %s for %s",
                                    cleanerName, ownerString);
                    LOG.error(message, e);
            ...
            try {
                ...
                Method waitForReferenceProcessingMethod =
                        referenceClass.getDeclaredMethod(
                                waitForReferenceProcessingName, waitForReferenceProcessingArgTypes);
                waitForReferenceProcessingMethod.setAccessible(true);
                return new PendingCleanersRunner(
                        waitForReferenceProcessingMethod, waitForReferenceProcessingArgs);
    ...
    private static class PendingCleanersRunner {
        ...
        private PendingCleanersRunner(
                Method waitForReferenceProcessingMethod, Object[] waitForReferenceProcessingArgs) {
        ...
        private boolean tryRunPendingCleaners() throws InterruptedException {
            try {
                return (Boolean)
                        waitForReferenceProcessingMethod.invoke(
                                null, waitForReferenceProcessingArgs);
            } catch (IllegalAccessException | InvocationTargetException e) {
        ...
        private static void throwIfCauseIsInterruptedException(Throwable t)
        ...
        private static <T> T throwInvocationError(Throwable t, Method method) {
            String message =
                    String.format(
                            ...
                            PendingCleanersRunnerProvider.REFERENCE_CLASS, method.getName());
            LOG.error(message, t);
    ...
    private static class ReflectionUtils {
        ...
        private Class<?> findClass(String className) {
            try {
                ...
                throw new FlinkRuntimeException(
                        String.format(
                                "%s: Failed to find %s class",
                                logPrefix, className.split("\\.")[0]),
                        e);
            }
        }
        ...
        private Method findMethod(Class<?> cl, String methodName, Class<?>... parameterTypes) {
            try {
                ...
                throw new FlinkRuntimeException(
                        String.format(
                                "%s: Failed to find %s#%s method",
                                logPrefix, cl.getSimpleName(), methodName),
                        e);
            }
        }
    }

...
public class KeyedStream<T, KEY> extends DataStream<T> {
    ...
    public SingleOutputStreamOperator<T> reduce(ReduceFunction<T> reducer) {
        return transform(
                "Keyed Reduce",
                getType(),
                new StreamGroupedReduce<T>(
                        clean(reducer), getType().createSerializer(getExecutionConfig())));
    }

    /**
     * Applies a fold transformation on the grouped data stream grouped on by the given key
     * position. The {@link FoldFunction} will receive input values based on the key value. Only
     * input values with the same key will go to the same folder.
     *
     * @param folder The {@link FoldFunction} that will be called for every element of the input
     *     values with the same key.
     * @param initialValue The initialValue passed to the folders for each key.
     * @return The transformed DataStream.
    ...
    public <R> SingleOutputStreamOperator<R> fold(R initialValue, FoldFunction<T, R> folder) {
        ...
        return transform(
                "Keyed Fold", outType, new StreamGroupedFold<>(clean(folder), initialValue));
    }
    ...
    protected SingleOutputStreamOperator<T> aggregate(AggregationFunction<T> aggregate) {
        StreamGroupedReduce<T> operator =
                new StreamGroupedReduce<T>(
    ...
    public QueryableStateStream<KEY, T> asQueryableState(String queryableStateName) {
        ValueStateDescriptor<T> valueStateDescriptor =
                new ValueStateDescriptor<T>(UUID.randomUUID().toString(), getType());
    ...
    public <ACC> QueryableStateStream<KEY, ACC> asQueryableState(
            String queryableStateName, FoldingStateDescriptor<T, ACC> stateDescriptor) {
